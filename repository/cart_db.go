...
func (c cartRepositoryDb) GetCart(userId int) ([]CartGet, error) {
	sql := "select s.id, p.price, s.quantity, s.created_at, p.code, p.name, p.description, p.rating, p.image, s.user_id from carts as s inner join products p on s.product_id = p.id where s.user_id = ? and s.deleted_at is null"
